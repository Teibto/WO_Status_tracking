...
define(['N/query', 'N/log', './WOStatusTracking_Labels'], (query, log, Labels) => {
  ...
  /** Rollup 11-element status arrays across multiple rows. */
  function rollupCpArrays(arrays) {
    if (!arrays || arrays.length === 0) return Array(11).fill('na');
        ...
        SUM(
          CASE
            WHEN mdr.custrecord_mfg_com_mac_down_reason IS NOT NULL
              AND (mdr.custrecord_mfg_com_mac_down_min IS NULL
                   OR mdr.custrecord_mfg_com_mac_down_min = 0)
            THEN 1 ELSE 0
          END
  ...
  /**
   * Q5: Pre-WOC labor existence per task (by op-task id).
   * Returns a map of op_task_id → record count.
   * Join key: custrecord_mfg_prewoc_labor_task = custrecord_mfg_tm_ot (op task id).
   */
  function queryPreWocLabor(opTaskIds) {
    if (!opTaskIds || !opTaskIds.length) return {};
    const safe = opTaskIds.map(id => parseInt(id, 10)).filter(id => !isNaN(id));
    if (!safe.length) return {};
    const sql = `
      SELECT
        lb.custrecord_mfg_prewoc_labor_task AS op_task_id,
        COUNT(lb.id)                        AS labor_count
      FROM customrecord_mfg_prewoc_labor lb
      WHERE lb.custrecord_mfg_prewoc_labor_task IN (${safe.join(',')})
      GROUP BY lb.custrecord_mfg_prewoc_labor_task
    `;
    const rows = runSQL(sql, []);
    const map = {};
    rows.forEach(r => { map[String(asNum(r.op_task_id))] = asNum(r.labor_count); });
    return map;
  }
   ...
   * @param {boolean} hasPreWocLabor  True if prewoc_labor records exist for this task's op_task
   ...
   */
  function computeTaskCheckpoints(task, woc, macDetail, hasPreWocLabor, hasCostAlloc, ctx) {
    ...
    // ── CP4: Labor (Pre-WOC Labor existence) ─────────────────
    cp.push(hasPreWocLabor ? 'ok' : 'wait'); cd.push({});
    ...
    // ── CP9: Standard Cost Setup (master data — not queried in drilldown Phase 1) ──
    cp.push('na'); cd.push({});
  ...
  function getDrilldownData(woid) {
    ...
    const opTaskIds = taskRows.map(t => asNum(t.op_task_id)).filter(Boolean);
    const wocRows  = queryWOC(taskIds);
    const wocIds   = wocRows.map(w => asNum(w.woc_id));

    const macRows            = queryMachineDetails(wocIds);
    const prewocLaborByOpTask = queryPreWocLabor(opTaskIds);
    ...
    taskRows.forEach(taskRow => {
      ...
      const opTaskId        = asStr(taskRow.op_task_id);
      const hasPreWocLabor  = opTaskId ? (prewocLaborByOpTask[opTaskId] || 0) > 0 : false;
      ...
      const { cpStatuses, cpData } = computeTaskCheckpoints(
        taskRow, woc, macDetail, hasPreWocLabor, hasCA, taskCtx
      );
  ...
  function renderHtmlFromData(woid, data, lang) {
    ...
    (data.batches || []).forEach(batch => {
      ...
      const batchCpStatuses = batch.cpStatuses || Array(11).fill('na');
