      ...
      case 5: {
        ...
        if (status === 'err') {
          switch (d.errType) {
            case 'noTime':
              return th
                ? 'ไม่มีข้อมูลเวลาแรงงาน'
                : 'Labor time not recorded';

            case 'totalMismatch':
              return th
                ? `เวลาแรงงานรวม ${d.recorded} ไม่ตรงกับผลรวมรายบุคคล ${d.detail}`
                : `Total labor time ${d.recorded} ≠ sum of individual records ${d.detail}`;

            default:
              return th ? 'ข้อมูลแรงงานผิดปกติ' : 'Labor data issue';
          }
        }
        return '';
      }
