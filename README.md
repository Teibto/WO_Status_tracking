...
ช่วงวันที่ยาวได้ถึง **92 วัน** (เดิมจำกัด 7 วัน — ยกที่ #78) · CP1 แบ่งหน้า**ที่ชั้น SQL** และ
CP2–CP9 ประมวลผลเฉพาะ WO ในหน้าปัจจุบัน ภาระงานจึงผูกกับขนาดหน้า ไม่ใช่จำนวน WO ทั้งช่วง ·
KPI 4 ใบจึงนับเฉพาะหน้าปัจจุบัน และมีบรรทัด "ใบสั่งผลิตทั้งช่วง: N" กำกับเสมอ
...
## สูตรลับใน BOM — WO Status แสดงชื่อวัตถุดิบให้ทุก role โดยตั้งใจ

> **คำตัดสิน:** ยอมรับตามที่เป็น · **ผู้ตัดสิน:** เจ้าของงาน (jurapa@teibto.com) ·
> **วันที่:** 2026-09-08 · **ที่มา:** issue #29 (ปิดด้วยคำตัดสินนี้ ไม่ได้แก้โค้ด)

**อ่านข้อนี้ก่อนแตะ CP3** — ไม่ใช่ของที่หลงมา เป็นของที่ตัดสินแล้วว่ารับไว้

หมายเหตุของ CP3 พิมพ์ **ชื่อวัตถุดิบที่ยังไม่ได้ป้อน** ออกหน้าจอสองที่

| ที่ | โค้ด | แสดงอะไร |
|---|---|---|
| หมายเหตุ CP3 ในตารางหลัก | `WOStatusTracking.js:790` (`computeCP3`) | 5 ชื่อแรก แล้วต่อด้วยจำนวนที่เหลือ |
| หน้าเจาะลึกระดับ task | `WOStatusTracking_Drilldown.js:388` → `WOStatusTracking_Labels.js:189` | ทั้งรายการ ไม่ตัดที่ 5 |

ชื่อมาจาก `item.displayname` ใน `getCP3a_BomComponents` / `getCP3b_FedItems`
(`WOStatusTracking_Queries.js:387` · `:428`) ซึ่ง**ไม่มีเงื่อนไขเรื่องชั้นความลับ** ·
ผู้เปิดรายงานได้ทุก role (`allroles=T`)

**ที่อื่นในบัญชีถือว่านี่เป็นความลับ** — `cseg_subitemtype = 6` แปลว่าสูตรลับ · PWOC และ MRP
ซ่อน BOM ที่เข้าเงื่อนไขนี้ผ่านโมดูลเดียวกัน (`SL-PWOC-BomRestrict.js` ← port จาก
`S_BomRestrict.js` ของ MRP) ที่อ่านกติกาจาก 2 custom record — `customrecord_ts1_bomrestrict_rule`
(อะไรลับ) และ `customrecord_ts1_bomrestrict_access` (ใครเห็นได้) · **ไม่ใช่ role ฮาร์ดโค้ด**
คู่มือส่งต่ออยู่ที่ `MRP/TEIBTO - TS1-MRP-Demand-Planning_fs/Docs/guides/KT_BomRestrict_Reuse.md`

**ทำไมรับไว้แทนที่จะปิดชื่อทันที** — ด่านที่พึ่ง role ตอนนี้ยังพิสูจน์ไม่ได้ว่ากันอะไรจริง

- ~~`runasrole=ADMINISTRATOR` บน SB1 ทำให้ยังไม่รู้ว่า `runtime.getCurrentUser().role`
  คืน role ของคนเปิดหรือ role ที่ script รันอยู่~~ — **ข้อนี้ปลดแล้วที่ #24 (2026-09-12)**
  `runasrole` เป็นค่าว่าง script จึงรันด้วย role ของคนเปิดจริง · เหลือข้อถัดไปข้อเดียว
  ที่ยังค้าง และมันคือข้อที่ทำให้คำตัดสินยังไม่เปลี่ยน
- ผลทดสอบ SB1 2026-07-20 (บันทึกที่ `MRP/.../Docs/issues/ISSUE_BomRestrict_Access_Review.md`)
  พบว่า **subsidiary ว่างทั้ง rule และ access** → ส่ง subsidiary ที่เลือกเข้าไปแล้ว `loadRule`
  ไม่ match → คืน null → **fail-open เงียบ ๆ** · ฟีเจอร์นี้จึงยังไม่ทำงานแม้ในฝั่ง MRP เอง
- ปิดชื่อโดยพึ่ง `BomRestrict` ที่ยัง fail-open อยู่ = ได้ตัวควบคุมที่ดูเหมือนควบคุม
  แต่ยังไม่รู้ว่าควบคุมจริงไหม ทั้งทิศ "ปล่อยผ่านหมด" และทิศ "ซ่อนจากทุกคนรวม R&D"

**ถ้าวันหนึ่งต้องปิดจริง** ทำสองขั้นตามลำดับ อย่าเริ่มที่ขั้น 2

1. **ตัดชื่อตามชั้นความลับของ WO — ไม่ต้องพึ่ง role เลย** · CP1 join `item` ของ mainline (= FG)
   อยู่แล้วที่ `WOStatusTracking_Queries.js:297` → เติม `item.cseg_subitemtype` ใน SELECT
   แล้วให้ CP3 ของ WO นั้นแสดงแค่จำนวน ไม่มีชื่อ · ⚠ **ตัวตัดสินคือ FG ที่ WO ผลิต
   ไม่ใช่วัตถุดิบแต่ละตัว** — PWOC เขียนกำกับไว้ที่ `SL-PWOC-FeedMat.js:37-44` ว่าหนึ่ง WO
   ซ่อนทั้งใบหรือเห็นทั้งใบ ทำไม่เหมือนกันคือได้คำตอบคนละอย่างกับ PWOC ทันที
2. **เสียบชั้น whitelist** (`BomRestrict.evaluate()`) เป็นเงื่อนไข "ถ้า `canView` ให้แสดงชื่อ"
   — #24 ปิดแล้ว เหลือเงื่อนไขเดียวคือ config subsidiary ของ rule ต้องถูกแก้ก่อน · ต้องให้ role ที่เปิด
   รายงานมีสิทธิ์ READ บน custom record ทั้งสองใบ ไม่งั้น query ล้มแล้วซ่อนเงียบ ๆ
...
ข้อจำกัดของตัวรายงานที่ยังจริง: ไม่มีการ flag WO ที่ค้างโดยไม่มี activity ·
ไม่มี filter "เฉพาะที่มีปัญหา" · ไม่มี sort ตามคอลัมน์ (ดู #80 — expand-all ทำแล้ว รอบ #80)
