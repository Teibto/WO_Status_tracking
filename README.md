...
- `runasrole=ADMINISTRATOR` บน SB1 (issue #24 ยังไม่ปิด) ทำให้ยังไม่รู้ว่า
  `runtime.getCurrentUser().role` คืน role ของคนเปิดหรือ role ที่ script รันอยู่
...
- ปิดชื่อโดยพึ่งของสองอย่างนั้น = ได้ตัวควบคุมที่ดูเหมือนควบคุมแต่ยังไม่รู้ว่าควบคุมจริงไหม
  ทั้งทิศ "ปล่อยผ่านหมด" และทิศ "ซ่อนจากทุกคนรวม R&D"
...
2. **เสียบชั้น whitelist** (`BomRestrict.evaluate()`) เป็นเงื่อนไข "ถ้า `canView` ให้แสดงชื่อ"
   — ทำได้หลัง #24 ปิด **และ** config subsidiary ของ rule ถูกแก้แล้ว · ต้องให้ role ที่เปิด
